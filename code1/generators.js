...
function GenerateDataset(n) {
     ...
     for (let i = 0; i < n; i++) {
          ...
          m = m.add(1, 'M');
          dataset.push({
               id: i + 1, price: getRandomInt(4), timestamp: m.format('L') + " [" + ((hours > 9) ? hours : ("0" + hours)) +
...
function test(n) {
     ...
     for (let i = 0; i < n; i++) {
          m = m.add(1, 'M');
          // // console.log('m :>> ', m.format('L'));
          // // console.log(getRandomInt(4));
          dataset.push({id: i+1, price: getRandomInt(4), timestamp: m + " [" + ((hours > 9) ? hours : ("0" + hours)) +
          ":" + ((minutes > 9) ? minutes : ("0" + minutes)) + ":" + ((seconds > 9) ? seconds : ("0" + seconds)) + "]" });
     }
}
